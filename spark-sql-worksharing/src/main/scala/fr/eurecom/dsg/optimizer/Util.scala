...
object Util {
  ...
  def isSameRelation(relationA:LogicalRelation, relationB:LogicalRelation):Boolean={
    ...
    relationB.relation match {
      case r:HadoopFsRelation=> r.inputFiles.foreach(pathsB += _.trim) // ParquetRelation or JSONRelation
      case _ => throw new IllegalArgumentException("unknown relation")
    }
  ...
  def computeTreeHashExternal(plan:LogicalPlan):BigInteger={
    ...
    plan match {
      ...
      case u:Union =>{
        val childrenHash = u.children.map(c => computeTreeHashExternal(c)).mkString(" ")
        // Union should consider combining Filters, not Projects
        Util.hash(className + childrenHash)
      }
      // ================================================================
