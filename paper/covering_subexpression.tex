...
Given a group of n SEs detected in phase 1, without our optimization, the cost of executing them is simply the sum of executing individual SE: $C = \sum _{i=1}^{n}C_{E}^{se_i}$. In contrast, by building a CE and use it as a \emph{cache plan}, it becomes: 
\[C^* = C_{E}^{ce} + C_{W}^{ce} + \sum _{i=1}^{n}(C_{R}^{ce} + C_{COMP}^{se_i})\]

CE will be executed and materialized to RAM once. Then the result is read back to answer each individual SE. Only when $C^* < C$, caching CE brings some benefit in reducing the total job's running time. 
