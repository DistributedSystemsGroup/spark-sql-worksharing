...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\For{each node $u$ in $T$}
			\State compute fingerprint $FP(u)$ using (1) and (2)
			\State put $(u, FP(u))$ into $HT$
		\EndFor
		\State \Return  $HT$
		\EndProcedure
	\end{algorithmic}
...
\For{each tree $T_{i}, i = 1..N$}
	...
	\For{each node $u$ in $T_{i}$ follow the DFS traversal}
		...
		\EndIf
		\State $AllowedMatching = True$	
		\If {$IsMatched$ $AND$ the tree rooted at u does not have cache-unfriendly operator}
